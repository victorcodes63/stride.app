...
function DepartmentsPageInner() {
  ...
  const totals = useMemo(() => {
    ...
    const staffCount = departments.reduce((s, d) => s + (d.employeeCount ?? 0), 0);
  ...
  const topDepartment = useMemo(() => {
    ...
    return departments
      .slice()
      .sort((a, b) => (b.employeeCount ?? 0) - (a.employeeCount ?? 0))[0];
  }, [departments]);
  ...
  return (
    <div className="w-full min-w-0">
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white p-5 sm:p-6 shadow-sm space-y-6">
        ...
        {clientId && selectedClient && !loadingDepts && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 pt-4">
            <div className="relative overflow-hidden rounded-2xl border border-neutral-200/90 bg-white p-4 sm:p-5 shadow-sm min-w-0">
              ...
              <p className="text-2xl sm:text-3xl font-bold text-primary-900 tabular-nums">{totals.deptCount}</p>
              <p className="text-[11px] text-neutral-500 mt-1">For this client</p>
            </div>
            <div className="relative overflow-hidden rounded-2xl border border-neutral-200/90 bg-white p-4 sm:p-5 shadow-sm min-w-0">
              <p className="text-[10px] sm:text-[11px] font-bold uppercase tracking-widest text-neutral-500 mb-1">Staff assigned</p>
              <p className="text-2xl sm:text-3xl font-bold text-emerald-700 tabular-nums">{totals.staffCount}</p>
              <p className="text-[11px] text-neutral-500 mt-1">Across departments</p>
            </div>
            <div className="relative overflow-hidden rounded-2xl border border-neutral-200/90 bg-white p-4 sm:p-5 shadow-sm min-w-0">
              <p className="text-[10px] sm:text-[11px] font-bold uppercase tracking-widest text-neutral-500 mb-1">Top department</p>
              {topDepartment ? (
                <>
                  <p className="text-xl sm:text-2xl font-bold text-primary-900 tabular-nums truncate">{topDepartment.name}</p>
                  <p className="text-[11px] text-neutral-500 mt-1">{topDepartment.employeeCount} staff</p>
                  <button
                    type="button"
                    onClick={() => setSearchQuery(topDepartment.name)}
                    className="mt-3 inline-flex items-center justify-center px-3 py-2 rounded-lg border border-neutral-200 bg-white text-sm font-medium text-neutral-700 hover:bg-neutral-50"
                  >
                    Filter list
                  </button>
                </>
            ...
            <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="relative max-w-md w-full">
                <input
                  ...
                  className="w-full pl-4 pr-3 py-2.5 border border-neutral-300 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 bg-white"

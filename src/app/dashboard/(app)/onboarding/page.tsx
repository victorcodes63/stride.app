...
export default function OnboardingPage() {
 ...
 <td className="col-center tabular-nums px-4 py-3">{new Date(row.startedAt).toLocaleDateString()}</td>
 <td className="col-center tabular-nums px-4 py-3">{row.complete}/{row.total}</td>
 <td className={`col-center px-4 py-3 ${row.overdue > 0 ? 'text-red-700 font-semibold' : ''}`}>{row.overdue}</td>
 <td className="col-center px-4 py-3">{row.status}</td>
 </tr>
 ))}
 </tbody>
 </table>

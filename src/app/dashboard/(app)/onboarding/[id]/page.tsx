...
export default function OnboardingDetailPage() {
  ...
  if (!data) return <div className="rounded-lg border bg-white p-4 text-sm text-neutral-600">Loading workflow...</div>;
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-lg border bg-white p-4">
        <h1 className="text-xl font-semibold text-primary-900">{data.employee.firstName} {data.employee.lastName}</h1>
        <p className="text-sm text-neutral-600">{data.employee.department?.name ?? 'No department'} • {data.type}</p>
        <div className="mt-3 h-2 rounded bg-neutral-200">
          <div className="h-2 rounded bg-primary-600" style={{ width: `${progress}%` }} />
        </div>
        <p className="mt-1 text-xs text-neutral-500">{completed}/{data.tasks.length} complete</p>
      </div>
